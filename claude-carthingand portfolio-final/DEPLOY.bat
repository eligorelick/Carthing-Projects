...
title Deploy Combined Dashboard to Car Thing
color 0B
cls
echo.
echo  ========================================
echo   DEPLOY COMBINED DASHBOARD
echo  ========================================
echo.
echo  This will deploy the all-in-one dashboard
echo  with Claude usage, Portfolio, and 2048.
echo.
...
scp -o StrictHostKeyChecking=no "%~dp0combined-display.html" root@%CARTHING_IP%:/opt/custom/combined-display.html 2>nul
...
if !SCP_RESULT! EQU 0 (
    ...
    ssh -o StrictHostKeyChecking=no root@%CARTHING_IP% "sed -i 's|/opt/custom/[a-zA-Z0-9_-]*\.html|/opt/custom/combined-display.html|g' /etc/sv/chromium/run" 2>nul
    ...
    echo  combined dashboard within a few seconds.
    echo.
    echo  DASHBOARD FEATURES:
    echo   - Button 1: Claude usage monitor
    echo   - Button 2: Portfolio holdings
    echo   - Button 3: Portfolio chart
    echo   - Button 4: 2048 game
    echo.
