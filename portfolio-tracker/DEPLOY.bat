...
title Deploy Portfolio Tracker to Car Thing
color 0B
cls
echo.
echo  ========================================
echo   DEPLOY TO CAR THING
echo  ========================================
echo.
echo  This will copy the Portfolio Tracker
echo  display to your Car Thing and activate it.
echo.
...
scp -o StrictHostKeyChecking=no "%~dp0portfolio-display.html" root@%CARTHING_IP%:/opt/custom/portfolio-display.html 2>nul
...
if !SCP_RESULT! EQU 0 (
    ...
    ssh -o StrictHostKeyChecking=no root@%CARTHING_IP% "sed -i 's|/opt/custom/[a-zA-Z0-9_-]*\.html|/opt/custom/portfolio-display.html|g' /etc/sv/chromium/run" 2>nul
    ...
    echo  ========================================
    echo.
    echo  Your Car Thing should now display your
    echo  portfolio tracker within a few seconds.
    echo.
